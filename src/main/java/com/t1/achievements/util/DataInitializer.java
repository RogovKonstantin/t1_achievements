...
public class DataInitializer {
    ...
    private record AchSpec(String code, String title, String shortDesc, boolean manual,
                           List<String> sectionCodes, List<String> tagCodes, List<Crit> criteria) {}
...
// 5) Популяция обычных пользователей
        ...
        // ---- TAGS
        Map<String, Tag> tags = saveTags(
                Map.of(
                        "lms", new Tag(null,"lms","Обучение"),
                        "cyber", new Tag(null,"cyber","Киберспорт"),
                        "music", new Tag(null,"music","Музыка"),
                        "docs", new Tag(null,"docs","Документы"),
                        "profile", new Tag(null,"profile","Профиль"),
                        "hr", new Tag(null,"hr","HR"),
                        "community", new Tag(null,"community","Сообщество"),
                        "writing", new Tag(null,"writing","Публикации")
                )
        );

        // ---- ASSETS (иконки/баннеры-заглушки)
        ...
        // ---- ACTIVITY TYPES (под все критерии)
        ...
        List<AchSpec> specs = List.of(
                ...
                new AchSpec("EDU_ARCH","Архитектор образования","Помощь в разработке обучающих программ", true,
                        List.of("LEARN","COMMUNITY"), List.of("community","lms"),
                ...
                new AchSpec("AUTHOR_T1","Автор Т1","Первая статья в профильном издании", true,
                        List.of("COMMUNITY"), List.of("writing","community"),
        ...
        for (AchSpec spec : specs) {
            ...
            a.setTags(spec.tagCodes().stream().map(tags::get).collect(Collectors.toSet()));
            ...
            // демонстрационные стадии для пары ачивок (необязательно)
        ...
        // ---- ACTIVITY LOGS (разнообразные события на 2 года назад)
        ...
        for (User u : allUsers) {
            ...
            // Подписант — генерим N месяцев подряд (0..12)
            ...
            // Ручные активности (случайным образом)
    ...
    private Map<String, Tag> saveTags(Map<String, Tag> map) {
        Map<String, Tag> out = new LinkedHashMap<>();
        for (var e : map.entrySet()) out.put(e.getKey(), tagRepo.save(e.getValue()));
        return out;
    }
    ...
    private void computeProgressAndAwards(List<User> users,
        ...
        // индексируем логи для ускорения
        ...
        for (User u : users) {
            ...
            for (AchSpec spec : specs) {
                ...
                for (Crit c : spec.criteria()) {
                    ...
                    // фильтруем по withinDays, если задано
                ...
                // автонаграда: если не manual и 100%
                ...
                // для manual: часть пользователей наградим вручную
